...
void VMEController::init() 
{
  ...
  //
  int open = 1; 
  theSocket=do_schar(open); // register a new schar device
  //
//   std::cout << "VMEController opened socket = " << theSocket << std::endl;
//   std::cout << "VMEController is using eth" << port_ << std::endl;
//   std::cout << "VCC Config Parameters are:" << std::endl;
//   std::cout << "  ipAddress : " << ipAddress_ << std::endl;
//   std::cout << "  MAC_addr  : " << std::hex << (int) MAC_addr[0] << "-" << (int) MAC_addr[1] << "-" << (int) MAC_addr[2] << "-" << (int) MAC_addr[3] << "-" << (int) MAC_addr[4] << "-" << (int) MAC_addr[5] <<  std::endl;
//   std::cout << "  MCAST_1  : " << std::hex << (int) MCAST_1[0] << "-" << (int) MCAST_1[1] << "-" << (int) MCAST_1[2] << "-" << (int) MCAST_1[3] << "-" << (int) MCAST_1[4] << "-" << (int) MCAST_1[5] <<  std::endl;
//   std::cout << "  MCAST_2  : " << std::hex << (int) MCAST_2[0] << "-" << (int) MCAST_2[1] << "-" << (int) MCAST_2[2] << "-" << (int) MCAST_2[3] << "-" << (int) MCAST_2[4] << "-" << (int) MCAST_2[5] <<  std::endl;
//   std::cout << "  MCAST_3  : " << std::hex << (int) MCAST_3[0] << "-" << (int) MCAST_3[1] << "-" << (int) MCAST_3[2] << "-" << (int) MCAST_3[3] << "-" << (int) MCAST_3[4] << "-" << (int) MCAST_3[5] <<  std::endl;
//   std::cout << "  Dflt_Srv_MAC  : " << std::hex << (int) Dflt_Srv_MAC[0] << "-" << (int) Dflt_Srv_MAC[1] << "-" << (int) Dflt_Srv_MAC[2] << "-" << (int) Dflt_Srv_MAC[3] << "-" << (int) Dflt_Srv_MAC[4] << "-" << (int) Dflt_Srv_MAC[5] <<  std::endl;
//   std::cout << "  Ethernet CR  : " << std::hex <<(int) CR_ethernet <<  std::endl;
//   std::cout << "  Ext_FIFO CR  : " << std::hex << (int) CR_ext_fifo <<  std::endl;
//   std::cout << "  Rst_Misc CR  : " << std::hex << CR_res_misc <<  std::endl;
//   std::cout << "  VME(low) CR  : " << std::hex << CR_VME_low <<  std::endl;
//   std::cout << "  VME(high) CR : " << std::hex << CR_VME_hi <<  std::endl;
//   std::cout << "  BTO          : " << std::hex << CR_BUS_timeout <<  std::endl;
//   std::cout << "  BGTO         : " << std::hex << CR_BUS_grant <<  std::endl;
//   std::cout << "  Msg_Lvl      : " << std::dec << Msg_Lvl <<  std::endl;
//   std::cout << "  Warn_On_Shtdwn : " << std::boolalpha << Warn_On_Shtdwn <<  std::endl;
//   std::cout << "  Pkt_On_Startup : " << std::boolalpha << Pkt_On_Startup <<  std::endl;
...
std::string VMEController::GetWarn_On_Shtdwn() {
    //printf("exec GetWarn_On_Shtdown\n"); 
  std::string stmp;
  stmp="false";
...
void VMEController::start(int slot, int boardtype) {
    //printf("exec start (%d, %d)\n", slot, boardtype); 
  //
  if(!done_init_) init();
  //
  board=boardtype;
  vmeadd=slot<<19;
  //printf("vmeadd base adr: %16X, slot: %d\n", vmeadd, slot); 
...
//printf("in VMEController::do_vme. fcn=%d, baseadd=%08X\n",fcn,vmeadd);
 ...
 if(fcn==2){
   ...
   //printf("exec write vme_controller(%d,%x,%x,%x)\n",itrd[when],add_rice,it[0],rcv[0]);
 ...
 if(fcn==1 ){
   ...
   //printf("exec read vme_controller(%d,%x,%x,%x)\n",itrd[when],add_rice,tmp[1],rcv[1]);
   //Jinghua Liu to added extra byte swap for those modules use do_vme(TMB,CCB,MPC)
   ttt=rcv[0];
   rcv[0]=rcv[1];
   rcv[1]=ttt;
...
{
    //printf("exec  new_vme (%x,%x,%x,%x,%x)\n", fcn, vme, data, rcv[0], when); 
...
{ 
    //printf("exec  vcc_read_command (code=%x, n_words=%x, readback=%x\n", code, n_words, readback[0]); 
   int n, l, lcnt;
...
{ 
    //printf("exec  vcc_write_command (code=%x, n_words=%x, writedata=%x\n", code, n_words, writedata[0]); 
   int n,l,lcnt;
...
{
    //printf("VME_controller (irdwr=%x, ptr=%x, data=%x, rcv=%x\n", irdwr, ptr, data[0], rcv[0]); 
  /* irdwr:   
     0 bufread
     1 bufwrite 
     2 bufread snd  
