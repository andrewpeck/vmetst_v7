...
// Check that wdcnt is a multiple of 4, all TMB readouts are mod(4) for DDU error checking
	...
	dprintf(stderr,  "TMB wdcnt is a multiple of 4. wdcnt=%i\n",dmb_wdcnt);
...
// Short header-only mode
	...
	dprintf(stderr,  "TMB readout short header-only mode. wdcnt=%i\n",dmb_wdcnt);
	}
...
// Long header-only mode
	...
	dprintf(stderr,  "TMB readout long header-only mode. wdcnt=%i\n",dmb_wdcnt);
	}
...
// Long header with raw hits mode
	...
	dprintf(stderr,  "TMB readout long header with raw hits mode. wdcnt=%i\n",dmb_wdcnt);
	}
...
// Check error flags
	...
	if(header_ok)	fprintf(log_file,"Header format OK\n");
	else {			fprintf(log_file,"Header format has errors\n");
