...
// Prototypes
//------------------------------------------------------------------------------
	void vme_jtag_io_byte	(unsigned long &adr, int &ichain, int &nframes, char tms[], char tdi[], char tdo[], int &step_mode);
	void pause				(string s);

//------------------------------------------------------------------------------
	void i4_to_tdi(long int &i4, char tdi[], int &nbits, int &spi)
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
	void tdi_to_i4(char tdi[], long int &i4, int &nbits, int &spi)
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
{
	...
	int			devices_in_chain[mxchain]={2,2,2,5};	// Chain 0,1,2,3
...
// Set up TMS to take TAP from RTI to Shift IR
	chain_id=-1;
	if (ichain==0x8) chain_id = 0;						// TMB user prom chain
	if (ichain==0xC) chain_id = 0;						// TMB fpga chain
	if (ichain==0xD) chain_id = 1;						// RAT chain
	if (ichain==0x3) chain_id = 2;						// ALCT mez pgm chain
	if (ichain==0x2) chain_id = 2;						// ALCT mez cfg chain
	if (ichain==0x4) chain_id = 3;						// TMB mez FPGA+4 PROMs
